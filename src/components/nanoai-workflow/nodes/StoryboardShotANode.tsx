/**
 * 故事板分镜A 节点
 * 功能：文本输入 → 提示词优化（GLM-4.5-Air）→ 入队后台生成 → 输出分镜图片
 */

import { memo, useCallback, useState, useRef } from 'react'
import { Handle, Position } from 'reactflow'
import { Film, Loader2, CheckCircle2, AlertCircle, Play, Sparkles, RotateCcw } from 'lucide-react'
...
interface ResultPreviewProps {
  result?: StoryboardShotAData['result']
  status: NodeStatus
}
...
export const StoryboardShotANode = memo(({ id, data }: { id: string; data: StoryboardShotAData }) => {
  ...
  const handleOptimize = useCallback(async () => {
    if (!rawPrompt) {
      setLocalError('请先输入故事描述或连接上游文本节点')
      return
    }
    ...
    >
      <div className="space-y-3">
        ...
        <div className={cn(
          ...
          <p>故事板分镜A</p>
          <p className="mt-0.5">输入描述 → 优化提示词 → 生成分镜</p>
        </div>
      </div>
